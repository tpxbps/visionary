...
class PretrainerBase(object):
    ...
    def train(self, task_ext):
        ...
        train_nav_db = ReverieTextPathData(
            data_cfg.train_traj_files, data_cfg.vp2room_label_file, data_cfg.img_ft_file, data_cfg.rec_img_ft_file, data_cfg.obj_ft_file,
        ...
        val_nav_db = ReverieTextPathData(
            data_cfg.val_seen_traj_files, data_cfg.vp2room_label_file, data_cfg.img_ft_file, data_cfg.rec_img_ft_file, data_cfg.obj_ft_file,
            data_cfg.scanvp_cands_file, data_cfg.connectivity_dir, data_cfg.clip_ft_file, self.ins2img_file,
            image_prob_size=self.model_config.image_prob_size,
            image_feat_size=self.model_config.image_feat_size, 
            angle_feat_size=self.model_config.angle_feat_size,
        ...
        val2_nav_db = ReverieTextPathData(
            data_cfg.val_unseen_traj_files, data_cfg.vp2room_label_file, data_cfg.img_ft_file, data_cfg.rec_img_ft_file, data_cfg.obj_ft_file,
            data_cfg.scanvp_cands_file, data_cfg.connectivity_dir,data_cfg.clip_ft_file, self.ins2img_file,
            image_prob_size=self.model_config.image_prob_size,
            image_feat_size=self.model_config.image_feat_size, 
            angle_feat_size=self.model_config.angle_feat_size,
    ...
    def validate_sap(self, model, val_loader):
        ...
        val_rloss, val_gloss, val_lloss, val_floss = 0, 0, 0, 0
        n_rcorrect, n_gcorrect, n_lcorrect, n_fcorrect = 0, 0, 0, 0
        n_data = 0
        st = time.time()
        for i, batch in enumerate(val_loader):
            rec_logits, global_logits, local_logits, fused_logits, global_act_labels, local_act_labels = \
                model(batch, task='sap', compute_loss=False)

            val_rloss += F.cross_entropy(rec_logits, local_act_labels, reduction='sum').data.item()
            ...
            n_rcorrect += torch.sum(torch.argmax(rec_logits, 1) == local_act_labels).item()
        ...
        val_rloss = sum(all_gather(val_rloss)) / n_data
        ...
        racc = sum(all_gather(n_rcorrect)) / n_data
        gacc = sum(all_gather(n_gcorrect)) / n_data
        lacc = sum(all_gather(n_lcorrect)) / n_data
        facc = sum(all_gather(n_fcorrect)) / n_data
        
        tot_time = time.time()-st
        val_log = {'rec_loss': val_rloss, 'gloss': val_gloss, 'lloss': val_lloss, 'floss': val_floss,
                'gacc': gacc, 'lacc': lacc, 'facc': facc,
                'tok_per_s': n_data/tot_time}
        LOGGER.info(f"validation finished in {int(tot_time)} seconds, "
                    f"racc: {racc*100:.2f}, gacc: {gacc*100:.2f}, lacc: {lacc*100:.2f}, facc: {facc*100:.2f}")
        return val_log
