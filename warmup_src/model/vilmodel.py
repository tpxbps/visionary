...
class ImageEmbeddings(nn.Module):
    ...
    def forward_rec(
        self, traj_view_img_fts, traj_rec_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types,
        traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, type_embed_layer
    ):
        device = traj_view_img_fts.device
        device2 = traj_rec_view_img_fts.device
        has_obj = traj_obj_img_fts is not None
    
        traj_view_img_embeds = self.img_layer_norm(self.img_linear(traj_view_img_fts))
        traj_rec_view_img_embeds = self.img_layer_norm(self.img_linear(traj_rec_view_img_fts))
        ...
        if has_obj:
            ...
            for view_embed, rec_view_embed, obj_embed, view_len, obj_len in zip(
                traj_view_img_embeds, traj_rec_view_img_embeds, traj_obj_img_embeds, traj_vp_view_lens, traj_vp_obj_lens
            ):
                if obj_len > 0:
                    traj_img_embeds.append(torch.cat([view_embed[:view_len], obj_embed[:obj_len]], 0))
            ...
            traj_img_embeds = traj_view_img_embeds
            rec_img_embeds = traj_rec_view_img_embeds
            traj_vp_lens = traj_vp_view_lens
        ...
        rec_embeds = rec_img_embeds + \
                      self.loc_layer_norm(self.loc_linear(traj_loc_fts)) + \
                      self.nav_type_embedding(traj_nav_types) + \
                      type_embed_layer(torch.ones(1, 1).long().to(device2))
        rec_embeds = self.layer_norm(rec_embeds)
        rec_embeds = self.dropout(rec_embeds)

        traj_masks = gen_seq_masks(traj_vp_lens)
        rec_masks = gen_seq_masks(traj_vp_lens)
        if self.pano_encoder is not None:
            traj_embeds = self.pano_encoder(
                traj_embeds, src_key_padding_mask=traj_masks.logical_not()
            )
            rec_embeds = self.pano_encoder(
                rec_embeds, src_key_padding_mask=rec_masks.logical_not()
            )

        split_rec_embeds = torch.split(rec_embeds, traj_step_lens, 0)
        split_traj_embeds = torch.split(traj_embeds, traj_step_lens, 0)
        split_traj_vp_lens = torch.split(traj_vp_lens, traj_step_lens, 0)
      
        return split_rec_embeds, split_traj_embeds, split_traj_vp_lens
...
class LocalVPEncoder(nn.Module):
    ...
    def vp_input_embedding_rec(self, split_rec_embeds, split_traj_embeds, split_traj_vp_lens, vp_pos_fts):
        vp_img_embeds = pad_tensors_wgrad([x[-1] for x in split_traj_embeds])
        rec_img_embeds = pad_tensors_wgrad([x[-1] for x in split_rec_embeds])
        vp_lens = torch.stack([x[-1]+1 for x in split_traj_vp_lens], 0)
        vp_masks = gen_seq_masks(vp_lens)
        max_vp_len = max(vp_lens)
        ...
        batch_size, _, hidden_size = rec_img_embeds.size()
        device2 = rec_img_embeds.device
        # add [stop] token at beginning
        rec_img_embeds = torch.cat(
            [torch.zeros(batch_size, 1, hidden_size).to(device2), rec_img_embeds], 1
        )[:, :max_vp_len]
     
        vp_embeds = vp_img_embeds + self.vp_pos_embeddings(vp_pos_fts)
        rec_embeds = rec_img_embeds + self.vp_pos_embeddings(vp_pos_fts)

        return vp_embeds, vp_masks, rec_embeds

    def forward_rec(
        self, txt_embeds, txt_masks, split_rec_embeds, split_traj_embeds, split_traj_vp_lens, vp_pos_fts
    ):
        vp_embeds, vp_masks, rec_embeds = self.vp_input_embedding_rec(
            split_rec_embeds, split_traj_embeds, split_traj_vp_lens, vp_pos_fts
        )
        vp_embeds = self.encoder(txt_embeds, txt_masks, vp_embeds, vp_masks)
        # rec_embeds = self.encoder(txt_embeds, txt_masks, rec_embeds, vp_masks)
        return vp_embeds, rec_embeds
...
class GlobalMapEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.use_rt_task = False
        if config.use_rt_task:
            self.use_rt_task = True
    ...
    def _aggregate_gmap_features(
        ...
        for i in range(batch_size):
            ...
            # set map node embedding  -> visited node mean embedding of panorama 
        ...
        return batch_gmap_img_fts # order == gmap_vpids order
    ...
    def forward(
        ...
        split_traj_embeds, split_traj_vp_lens, traj_vpids, traj_cand_vpids, gmap_vpids,
        gmap_step_ids, gmap_pos_fts, gmap_lens, gmap_vpids_mask=None, graph_sprels=None
    ):
        ...
        if gmap_vpids_mask is not None:
            gmap_vpids_mask_ext = gmap_vpids_mask.unsqueeze(-1).expand_as(gmap_embeds)
            gmap_embeds = gmap_embeds.data.masked_fill(gmap_vpids_mask_ext, 0)
    ...
    def forward_rt(self, split_traj_embeds, split_traj_vp_lens, traj_vpids, traj_cand_vpids, gmap_vpids,
                   gmap_step_ids, gmap_pos_fts, gmap_lens, gmap_vpids_mask,graph_sprels=None):
...
class GlocalTextPathCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        self.use_rt_task = config.use_rt_task
        if config.use_rt_task:
            self.global_visual_encoder = GlobalVisualEncoder(config)
    ...
    def forward(
            ...
            gmap_vpids_mask=None, return_gmap_embeds=True, rt_task=False,
    ):
        global CROP_SIZE
        ...
        max_step = max(traj_step_lens)

        for step in range(max_step):

            knowledge_ft = torch.cat(
                [knowledge_fts[b][min(step, knowledge_fts[b].shape[0] - 1):min(step, knowledge_fts[b].shape[0] - 1) + 1]
                 for b in range(batch_size)], dim=0)
            crop_ft = torch.cat(
                [crop_fts[b][min(step, crop_fts[b].shape[0] - 1):min(step, crop_fts[b].shape[0] - 1) + 1] for b in
            ...
            knowledge_ft_shape = knowledge_ft.shape
            crop_ft = self.crop_proj(crop_ft)
            crop_ft_shape = crop_ft.shape

            instruction_fts_pure = self.instruction_proj(txt_embeds).permute(0, 2, 1).unsqueeze(1).repeat(1, 36, 1, 1)

            # Purification
            if step != 0:
                history_matrix, _ = torch.matmul(history_fts, instruction_fts_pure).max(dim=-1)

            crop_matrix, _ = torch.matmul(crop_ft, instruction_fts_pure).max(dim=-1)
            knowledge_matrix, _ = torch.matmul(knowledge_ft, instruction_fts_pure).max(dim=-1)
            ...
            crop_ft = crop_ft * crop_purify_weight.unsqueeze(-1)
            knowledge_ft = knowledge_ft * knowledge_purify_weight.unsqueeze(-1)

            crop_knowledge_ft = self.cross_vision_knowledge(
                knowledge_ft.view(batch_size * 36, 1, 768), None,
                crop_ft.view(batch_size * 36, CROP_SIZE, 768), None).view(batch_size, 36, CROP_SIZE, 768)

            if step != 0:
                history_knowledge_ft = self.cross_history_knowledge(
                    knowledge_ft.view(batch_size * 36, 1, 768), None,
                    history_fts.view(batch_size * 36, -1, 768), None).view(batch_size, 36, -1, 768)

            # Fusion
            instruction_cls = self.fusion_proj(txt_embeds[:, 0:1, :]).permute(0, 2, 1).unsqueeze(1).repeat(1, 36, 1, 1)

            if step != 0:
                history_knowledge_logits = torch.matmul(history_knowledge_ft, instruction_cls) / math.sqrt(768)
                history_knowledge_weight = torch.softmax(history_knowledge_logits, dim=-2)
                history_knowledge_ft = (history_knowledge_ft * history_knowledge_weight).sum(-2)
                history_knowledge_ft = self.fusion_layer_norm(history_knowledge_ft)

            crop_knowledge_logits = torch.matmul(crop_knowledge_ft, instruction_cls) / math.sqrt(768)
            crop_knowledge_weight = torch.softmax(crop_knowledge_logits, dim=-2)
            crop_knowledge_ft = (crop_knowledge_ft * crop_knowledge_weight).sum(-2)
            crop_knowledge_ft = self.fusion_layer_norm(crop_knowledge_ft)

            # Fuse pano_fts, crop_knowledge_fts, history_knowledge_fts
            traj_ids = [min(sum(traj_step_lens[:b + 1]) + step, sum(traj_step_lens) - 1) for b in range(batch_size)]
            traj_ids = torch.tensor([step] + traj_ids[:-1]).cuda()
            ...
            if step != 0:
                ...
                fusion_fts = torch.cat(
                    [crop_knowledge_ft, self.no_history_embedding.unsqueeze(0).repeat(batch_size, 36, 1)], dim=-1)
            ...
            for i in range(batch_size):
                ...
                if step < len(used_cand_ids[i]):
                    for cand_id in used_cand_ids[i][step]:
                        traj_view_img_fts[traj_ids][i, view_ft_id] = fusion_fts[i, cand_id] + \
                                                                     traj_view_img_fts[traj_ids][i, view_ft_id]
                        view_ft_id += 1
        ...
        if return_gmap_embeds:
            gmap_embeds = self.global_encoder(
                ...
                split_traj_embeds, split_traj_vp_lens, traj_vpids, traj_cand_vpids, gmap_vpids,
                gmap_step_ids, gmap_pos_fts, gmap_lens, gmap_vpids_mask, graph_sprels=gmap_pair_dists,
            )
        else:
            gmap_embeds = None
    ...
    def forward_rec(
        self, txt_ids, txt_lens, traj_view_img_fts, traj_rec_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types,
        traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
        gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, vp_pos_fts, knowledge_fts, crop_fts, used_cand_ids,
        gmap_vpids_mask=None, return_gmap_embeds=True, rt_task=False,
    ):
        ...
        # trajectory embedding
        # todo: add rec_traj_view_img_fts -> rec_split_traj_embeds
        split_rec_embeds, split_traj_embeds, split_traj_vp_lens = self.img_embeddings.forward_rec(
            traj_view_img_fts, traj_rec_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types,
            traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens,
            self.embeddings.token_type_embeddings
        )
        ...
        # vp embeds
        # todo: add rec_split_traj_embeds->rec_embeds
        vp_embeds, rec_embeds = self.local_encoder.forward_rec(
            txt_embeds, txt_masks,
            split_rec_embeds, split_traj_embeds, split_traj_vp_lens, vp_pos_fts
        )

        return gmap_embeds, vp_embeds, rec_embeds
    ...
    def forward_mlm(
        ...
        # todo： add caption enhanced module
        max_step = max(traj_step_lens)
        ...
        # trajectory embedding   view_img_fts -> pano feature
        split_traj_embeds, split_traj_vp_lens = self.img_embeddings(
            traj_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, 
            traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens,
            self.embeddings.token_type_embeddings
