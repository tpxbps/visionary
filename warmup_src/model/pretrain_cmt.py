...
class GlocalTextPathCMTPreTraining(BertPreTrainedModel):
    def __init__(self, config):
        ...
        if 'sap' in config.pretrain_tasks:
            ...
            self.local_rec_reg_head = NodeDistReg(input_size=self.config.hidden_size, config=config)  # add
            self.local_rec_sap_head = ClsPrediction(self.config.hidden_size)  # add
    ...
    def forward(self, batch, task, compute_loss=True):
        ...
        if task.startswith('mlm'):
            ...
            return self.forward_sap(
                batch['txt_ids'], batch['txt_lens'], batch['traj_view_img_fts'], batch['traj_rec_view_img_fts'],
                batch['traj_obj_img_fts'], batch['traj_loc_fts'], batch['traj_nav_types'], 
                batch['traj_step_lens'], batch['traj_vp_view_lens'], batch['traj_vp_obj_lens'], 
                batch['traj_vpids'], batch['traj_cand_vpids'], 
                batch['gmap_lens'], batch['gmap_step_ids'], batch['gmap_pos_fts'], 
            ...
            return self.forward_dsap(
                ...
                batch['ins2img_feat'], batch['current_vpid_index'], batch['knowledge_fts'], batch['crop_fts'], batch['used_cand_ids'], compute_loss,
            )
    ...
    def forward_sap(
        ...
        gmap_embeds, vp_embeds, rec_embeds = self.bert.forward_rec(
            txt_ids, txt_lens, traj_view_img_fts, traj_rec_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types,
            traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
            gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, vp_pos_fts, knowledge_fts, crop_fts, used_cand_ids,
        )

        _, rec_logits, fuse_weight3 = self.local_rec_reg_head(vp_embeds, rec_embeds)
        ...
        fused_logits3.masked_fill_(vp_nav_masks, -float('inf'))

        rec_logits_copy = torch.clone(rec_logits)
        rec_logits_copy.masked_fill_(vp_nav_masks, -float('inf'))
        ...
        fused_logits_rec = torch.clone(global_logits)
        fused_logits_rec[:, 0] += fused_logits3[:, 0]  # stop
        for i in range(batch_size):
            visited_nodes = set([vp for vp, mask in zip(gmap_vpids[i], gmap_visited_masks[i]) if mask])
            tmp = {}
            bw_logits = 0
            for j, cand_vpid in enumerate(traj_cand_vpids[i][-1]):
                if cand_vpid in visited_nodes:
                    bw_logits += fused_logits3[i, j + 1]
                else:
                    tmp[cand_vpid] = fused_logits3[i, j + 1]
            for j, vp in enumerate(gmap_vpids[i]):
                if j > 0 and vp not in visited_nodes:
                    if vp in tmp:
                        fused_logits_rec[i, j] += tmp[vp]
                    else:
                        fused_logits_rec[i, j] += bw_logits

        # fused_logits += fused_logits3
        fused_logits += fused_logits_rec

        if compute_loss:
            rec_losses = F.cross_entropy(rec_logits_copy, local_act_labels, reduction='none')
            ...
            losses = global_losses + local_losses + fused_losses + rec_losses
            return losses
        else:
            return rec_logits_copy, global_logits, local_logits, fused_logits, global_act_labels, local_act_labels
    ...
    def forward_rt(
        ...
        gmap_embeds, _  = self.bert.forward_rt(txt_ids, txt_lens, traj_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types,
                    traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
                    gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, vp_pos_fts,
    ...
    def forward_dsap(
        ...
        traj_step_lens, traj_vp_view_lens, traj_vp_obj_lens, traj_vpids, traj_cand_vpids,
        gmap_lens, gmap_step_ids, gmap_pos_fts, gmap_pair_dists, gmap_vpids, vp_pos_fts,
        gmap_visited_masks, global_act_labels, local_act_labels, ins2img, current_vpid_index, knowledge_fts, crop_fts, used_cand_ids,
        compute_loss 
    ):  
        ...
        # todo: _, img_logits, fuse_weight3 = self.node_dis_reg_head(vp_embeds, d_img_fts)
        _, rec_logits, fuse_weight3 = self.local_rec_reg_head(vp_embeds, rec_embeds)
        ...
        if compute_loss:
            ...
            local_losses = F.cross_entropy(local_logits, local_act_labels, reduction='none')
            fused_losses = F.cross_entropy(fused_logits, global_act_labels, reduction='none')
            losses = global_losses + local_losses + fused_losses + dist_losses + rec_losses
            return losses
        else:
            return rec_logits_copy, dist_logits_copy, global_logits, local_logits, fused_logits, global_act_labels, local_act_labels
