...
class ReverieTextPathData(object):
    def __init__(
        self, anno_files, vp2room_file, img_ft_file, rec_img_ft_file, obj_ft_file, scanvp_cands_file, connectivity_dir,
        ...
        self.img_ft_file = img_ft_file
        self.rec_img_ft_file = rec_img_ft_file
        self.obj_ft_file = obj_ft_file
        self.vp2room_file = vp2room_file
        self.clip_ft_file = clip_ft_file
        self.ins2img_file = ins2img_file
        self.crop_db = FeaturesDB("../datasets/visionary/clip_crop_image.hdf5")
        self.knowledge_db = FeaturesDB("../datasets/visionary/captions.hdf5")
    ...
    # todo: add rec_view_fts from rec_img_ft_file
    def get_scanvp_feature(self, scan, viewpoint):
        key = '%s_%s' % (scan, viewpoint)
        if self.in_memory and key in self._feature_store:
            view_fts, rec_view_fts, obj_fts, obj_attrs = self._feature_store[key]
        else:
            with h5py.File(self.img_ft_file, 'r') as f:
                view_fts = f[key][...].astype(np.float32)
            # with h5py.File(self.rec_img_ft_file, 'r') as f:
            #     rec_view_fts = f[key][...].astype(np.float32)
                # ft = f[key][...][:, :self.image_feat_size].astype(np.float32)
            ...
            if len(self.clip_ft_file) != 0 :
                ...
                rec_view_fts = view_fts
                view_fts[:,:feat_size] = clip_view_fts
                with h5py.File(self.rec_img_ft_file, 'r') as f:
                    clip_rec_view_fts = f[key][...].astype(np.float32)
                rec_view_fts[:,:feat_size] = clip_rec_view_fts

            if self.in_memory:
                self._feature_store[key] = (view_fts, rec_view_fts, obj_fts, obj_attrs)

        return view_fts, rec_view_fts, obj_fts, obj_attrs
    ...
    def get_input(
        ...
        # todo: add rec_traj features of all path
        traj_view_img_fts, traj_rec_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids, \
        ...
        outs = {
            ...
            'traj_rec_view_img_fts': [x[:, :self.image_feat_size] for x in traj_rec_view_img_fts],
        ...
        if return_img_probs:
            ...
            outs['vp_rec_view_probs'] = softmax(traj_rec_view_img_fts[-1][:, self.image_feat_size:], dim=1)
    ...
    def get_traj_pano_fts(self, scan, path):
        ...
        used_cand_ids = []
        for vp in path:
            view_fts, rec_view_fts, obj_img_fts, obj_attrs = self.get_scanvp_feature(scan, vp)

            view_img_fts, rec_view_img_fts, view_angles, cand_vpids = [], [], [], []
            # cand views
            ...
            for k, v in nav_cands.items():
                ...
                rec_view_img_fts.append(rec_view_fts[v[0]])
            ...
            rec_view_img_fts.extend([rec_view_fts[idx] for idx in range(36) if idx not in used_viewidxs])
            ...
            traj_rec_view_img_fts.append(rec_view_img_fts)
            ...
            for i in range(36):
                ...
                knowledge_feature.append(self.knowledge_db.get_feature(knowledge_ids).reshape(1, 512))
        ...
        return traj_view_img_fts, traj_rec_view_img_fts, traj_obj_img_fts, traj_loc_fts, traj_nav_types, traj_cand_vpids, \
