...
class Pretrainer(PretrainerBase):
    ...
    def init_model(self):
        ...
        if self.opts.checkpoint:
            ...
            if self.opts.init_pretrained == 'bert':
                ...
                print(f"Loading weights from {'../datasets/pretrained/LXMERT/model_LXRT.pth'}")
    ...
    def pretrain_phase1(self):
        ...
        last_ckpt_of_aug = '../ckpts/warmup_stage2.pt'
    ...
    def validate_distsap(self, model, val_loader):
        ...
        val_rloss, val_dloss, val_gloss, val_lloss, val_floss = 0, 0, 0, 0, 0
        n_rcorrect, n_dcorrect, n_gcorrect, n_lcorrect, n_fcorrect = 0, 0, 0, 0, 0
        n_data = 0
        st = time.time()
        for i, batch in enumerate(val_loader):
            rec_logits, dist_logits, global_logits, local_logits, fused_logits, global_act_labels, local_act_labels = \
        ...
        tot_time = time.time()-st
        val_log = {'rec_loss': val_rloss, 'dis_loss': val_dloss, 'gloss': val_gloss, 'lloss': val_lloss, 'floss': val_floss,
                'dacc': dacc,'gacc': gacc, 'lacc': lacc, 'facc': facc,
                'tok_per_s': n_data/tot_time}
        LOGGER.info(f"validation finished in {int(tot_time)} seconds, "
                    f"racc: {racc*100:.2f}, dacc: {dacc*100:.2f}, gacc: {gacc*100:.2f}, lacc: {lacc*100:.2f}, facc: {facc*100:.2f}")
        return val_log
