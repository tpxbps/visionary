...
class Ins2ImageFeaturesDB(object):
    def __init__(self, ins2img_ft_file, image_feat_size):
        self.image_feat_size = image_feat_size
        self.ins2img_ft_file = ins2img_ft_file
        self._feature_store = {}
    
    def get_ins2image_feature(self,instr_id):
        key = instr_id
        if key in self._feature_store:
            ft = self._feature_store[key]
        else:
            with h5py.File(self.ins2img_ft_file, "r") as f:
                ft = f[key][...][:, :self.image_feat_size].astype(np.float32)
                if len(ft.shape) == 4:
                    ft = np.squeeze(ft)
                self._feature_store[key] = ft 
...
def get_point_angle_feature(sim, angle_feat_size, baseViewId=0):
    ...
    for ix in range(36):
        if ix == 0:
            ...
            #first [0] don't move
            #second [1.0] turn heading (30 degree) (negative-> left, positive-> right)
            #thirs [0] turn elevation  (30 degree) (negative-> down, positive-> up )
        ...
        # print("===================================")
        # print(f"{ix} state_heading: {state.heading}, state_elevation: {state.elevation}")
        # print(f"{ix}  base_heading: {base_heading},   base_elevation: {base_elevation}")
        # print(f"{ix}       heading: {heading}              elevation: {elevation}")
        # print("===================================")
