...
class FloydGraph(object):
    ...
    def add_edge(self, x, y, dis):
        # x,y viewpointId  dis ecludi-> distance
    ...
    def update(self, k):
        ...
        for x in self._dis:
            for y in self._dis:
                if x != y:
                    #if dis(x->k->y) < dis(x->y)
                    # => dis(x->y) = dis(x->k->y) shortest path of x-->y
...
class GraphMap(object): 
    def __init__(self, start_vp):
        ...
        self.node_step_ids = {} # viewpoint: step 
...
class GraphRoomMap(object):
    ...
    def update_graph(self, ob):
        self.curr_id = ob['viewpoint']
        self.node_positions[ob['viewpoint']] = ob['position']
        self.node_room_type[ob['viewpoint']] = ob['room_type']
        self.node_dist[ob['viewpoint']] = ob['step_score_to_goal']

        for cc in ob['candidate']:
            self.node_positions[cc['viewpointId']] = cc['position']
            try:
                self.node_room_type[cc['viewpointId']] = cc['room_type']
                self.node_dist[cc['viewpointId']] = cc['step_score_to_goal']
            except:
                # set_trace()
                pass
            dist = calc_position_distance(ob['position'], cc['position'])
    ...
    def get_node_room_type(self, vp):
        return self.node_room_type[vp]
    
    def get_node_dist(self, vp):
        return self.node_dist[vp]
...
class HierarchicalGraphMap(object):
    ...
    def update_layout_graph_embeds(self):
        pass 
