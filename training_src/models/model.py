...
def get_vlnbert_models(args, config=None):
    ...
    vis_config = PretrainedConfig.from_pretrained('../bert-base')
...
class VLNBert(nn.Module):
    ...
    def forward(self, mode, batch):
        ...
        if mode == 'language':            
            ...
            pano_embeds, rec_pano_embeds, pano_masks, rec_pano_masks = self.vln_bert(mode, batch)
            ...
            outs = self.vln_bert(mode, batch)  # cur
            return outs
        else:
