...
def parse_args():
    ...
    parser.add_argument('--tokenizer', choices=['bert', 'xlm', 'clip'], default='bert')

    parser.add_argument('--fusion', choices=['global', 'local', 'avg', 'dynamic', 'fuse_ins_img'])
    ...
    parser.add_argument('--eval_ckpt_file', default=None, help='Ckpt for evaluation')
    ...
    parser.add_argument('--use_room_type', action='store_true', default=False)
    parser.add_argument('--num_v_layers', type=int, default=6)
    parser.add_argument('--num_layout_layers',type=int, default=6)
    parser.add_argument('--use_img_room_head',action='store_true',default=False)

    parser.add_argument('--use_gd', action='store_true', default=False)
    parser.add_argument('--use_real_dist_norm',  action='store_true', default=False)
    parser.add_argument('--node_loss_delta',  type=float, default=1.)
    parser.add_argument('--fuse_dist_score_to_global',type=float, default=0.)
    parser.add_argument(
        "--gd_dreamer_type",
        default='attn',
        type=str,
        help="gd dreamer encoder type",
    )
    ...
    parser.add_argument('--switch_first_gd',  action='store_true', default=False)
    ...
    parser.add_argument('--bw_weight',  type=float, default=0.)
    parser.add_argument('--avg_local_emb', action='store_true', default=False)
    parser.add_argument('--use_dist_logits_prediction', action='store_true', default=False)
    parser.add_argument('--stable_gd', action='store_true', default=False)
    parser.add_argument('--gd_feat_dropout', type=float, default=0.)
    ...
    parser.add_argument('--rt_weights', type=float, default=1.0)
    # Model hyper params:
    ...
    parser.add_argument('--h_graph', action='store_true', default=False)
    parser.add_argument('--rp_embed_dir', type=str, default=None)
    parser.add_argument('--num_of_ins_img',type=int, default=0)
    parser.add_argument('--global_fuse', action='store_true', default=False)
    parser.add_argument('--update_rp_embed', action='store_true', default=False)
...
def postprocess_args(args):
    ...
    ins2img_ft_file_map = {
        'vitbase': 'full_reverie_ins2img_imgnet.h5',
        'clip': 'full_reverie_ins2img_clip.h5'
    }
    args.img_ft_file = os.path.join(ROOTDIR, 'R2R', 'features', ft_file_map[args.features])
    args.ins2img_ft_file = os.path.join(ROOTDIR, 'REVERIE', 'features', ins2img_ft_file_map[args.features])

    args.rec_img_ft_file = os.path.join(ROOTDIR, 'R2R', 'features', 'rec_pth_clip_vit_l_14_336px.hdf5')
