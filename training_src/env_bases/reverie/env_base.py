...
class EnvBatch(object):
    ...
    def __init__(self, connectivity_dir, scan_data_dir=None, feat_db=None, rec_feat_db=None, batch_size=100):
        """
    ...
    def getStates(self):
        ...
        :return: [ ((36, 2048 [772]), sim_state) ] * batch_size
        """
        feature_states = []
        for i, sim in enumerate(self.sims):
            state = sim.getState()[0]
            feature = self.feat_db.get_image_feature(state.scanId, state.location.viewpointId)
            rec_feature = self.rec_feat_db.get_image_feature(state.scanId, state.location.viewpointId)
            # shape 36 * 768 => [3 * 12 = 36]  3 elevations
            feature_states.append((feature, rec_feature, state))
...
class ReverieObjectNavBatch(object):
    ...
    def __init__(
        ...
        mp3d_scan_dir = "../mp3d/v1/scans"
        self.env = EnvBatch(connectivity_dir, scan_data_dir=mp3d_scan_dir, feat_db=view_db, batch_size=batch_size)
        self.obj_db = obj_db
    ...
    def _next_minibatch(self, batch_size=None, **kwargs):
        ...
        if self.multi_endpoints: # True
            for i, item in enumerate(batch):
                end_vp = item['end_vps'][np.random.randint(len(item['end_vps']))]
                end_vps[i] = end_vp

        if self.multi_startpoints or self.multi_endpoints: #True
    ...
    def make_candidate(self, feature, scanId, viewpointId, viewId):
        ...
        # turn around 360 degree (30 degree each time) --> heading
        # elevation -> (-30, 0, 30) 
        # check navigable location from each view point 
            for ix in range(36):
                if ix == 0:
                    self.sim.newEpisode([scanId], [viewpointId], [0], [math.radians(-30)])
                elif ix % 12 == 0:
                    self.sim.makeAction([0], [1.0], [1.0]) # turn elevation
                else: 
                    self.sim.makeAction([0], [1.0], [0])  # turn heading
                ...
                visual_feat = feature[ix]  # select from image feats (360, -30,0,30)
    ...
    def _get_obs(self):
        obs = []
        for i, (feature, _, state) in enumerate(self.env.getStates()):
            item = self.batch[i]
