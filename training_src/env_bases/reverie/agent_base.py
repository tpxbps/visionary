...
# def KL_objective (input, target):
#     logprobs = F.log_softmax (input, dim = 1)
#     target = F.softmax (target, dim = 1)
#     # return  -(target * logprobs).sum() / input.shape[0]
#     return  F.kl_div(logprobs, target, reduction='batchmean')

def KL_objective (input, target):
    logprobs = F.log_softmax (input, dim = 1)
    target = F.softmax (target, dim = 1)
    # return  -(target * logprobs).sum() / input.shape[0]
    return  F.kl_div(logprobs, target, reduction='none')
...
class Seq2SeqAgent(BaseAgent):
    ...
    def __init__(self, args, env, rank=0):
        ...
        #     {'params': [p for n, p in update_param], 'lr': self.args.lr * 10.},  # 训练
        #     {'params': [p for n, p in freeze_param], 'lr': self.args.lr * 1e-3}  # 微调
        # ]
        #
        ...
        # self.criterion_kl = KL_objective
        #
        # # Logs
        # sys.stdout.flush()
